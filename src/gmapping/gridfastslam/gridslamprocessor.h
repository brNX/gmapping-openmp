...
struct Particle{
 ...
 @param map: the particle map
    ...
    Particle(const ScanMatcherMap& map):
        map(map), pose(0, 0, 0), weight(0), weightSum(0), gweight(0), previousIndex(
                                                                          0) {
        node = 0;
    }
...
#define isnan(x) (x==FP_NAN)
 ...
 /*   inline void scanMatch(double * reading) {
        // sample a new pose from each scan in the reference

        //double sumScore = 0;
        if  (m_count==1)
            pbufhelper.initWorkPackage(m_minimumScore,m_matcher);

        pbufhelper.setWorkPackageLaserReading(reading,m_beams);
        ...
        for (unsigned int i = 0; i < m_particles.size(); i++) {
            ...
            zmq::message_t workmessage(sizeof(unsigned int));
            * (uint32_t*)workmessage.data()=i;
            workqueue->send(workmessage);

        }

        //wait for sync  //TODO: change to score
        for (unsigned int i=0; i<m_particles.size();i++){
            zmq::message_t sync;
            syncqueue->recv(&sync);
        }
