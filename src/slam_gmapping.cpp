...
laser_count_(0), transform_thread_(NULL)
{
	...
	//BOOKMARK: rever utilidade disto
	transform_thread_ = new boost::thread(boost::bind(&SlamGMapping::publishLoop, this, transform_publish_period));

#ifdef LOGTOFILE
	//codigo do logger
	laserfile = fopen("laserlog","wb");
	odomfile  = fopen("odomlog","wb");
	posestampedfile  = fopen("posestampedlog","wb");
	laserposefile = fopen("laserposelog","wb");
	minmaxfile = fopen("minmaxlog","wb");
#endif
...
SlamGMapping::~SlamGMapping()
{
	...
	fclose(laserfile);
	fclose(odomfile);
	fclose(posestampedfile);
	fclose(laserposefile);
	fclose(minmaxfile);
#endif
}
...
SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
	...
	try
	{
		//BOOKMARK: write odom_pose to file
		tf_.transformPose(odom_frame_, ident, odom_pose);

		//logger
		//ROS_INFO_//BOOKMARK: write odom_pose to fileONCE("size of odom_pose %lu\n",sizeof(odom_pose));
		//fwrite(&odom_pose,sizeof(odom_pose),1,odomfile);
#ifdef LOGTOFILE
		writeOdomMsgtoFile(odom_pose);
#endif
	...
	//ROS_INFO_ONCE("yaw: %f",yaw);
	//ROS_INFO_ONCE("x: %f",odom_pose.getOrigin().x());
	//ROS_INFO_ONCE("y: %f",odom_pose.getOrigin().y());
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
    ...
    for(GMapping::TNode* n = best.node;
            n;
            n = n->parent)
    {
        ROS_DEBUG("  %.3f %.3f %.3f",
...
void SlamGMapping::writeHeader(const std_msgs::Header_<std::allocator<void> > & header, FILE * file){

	//Header
	fwrite(&header.seq,sizeof(uint32_t),1,file);
	fwrite(&header.stamp.sec,sizeof(uint32_t),1,file);
	fwrite(&header.stamp.nsec,sizeof(uint32_t),1,file);

}

//BOOKMARK: write scan to file
void SlamGMapping::writeLaserMsgtoFile(const sensor_msgs::LaserScan & scan){


	//HEADER
	writeHeader(scan.header,laserfile);

	fwrite(&scan.angle_min,sizeof(float),1,laserfile);
	fwrite(&scan.angle_max,sizeof(float),1,laserfile);
	fwrite(&scan.angle_increment,sizeof(float),1,laserfile);
	fwrite(&scan.time_increment,sizeof(float),1,laserfile);
	fwrite(&scan.scan_time,sizeof(float),1,laserfile);
	fwrite(&scan.range_min,sizeof(float),1,laserfile);
	fwrite(&scan.range_max,sizeof(float),1,laserfile);

	//ranges size

	unsigned long size = scan.ranges.size();

	fwrite(&size,sizeof(size),1,laserfile);

	for(unsigned int i=0;i<scan.ranges.size();i++)
		fwrite(&scan.ranges[i],sizeof(float),1,laserfile);

	//intensities size
	size=scan.intensities.size();
	fwrite(&size,sizeof(size),1,laserfile);

	//ROS_INFO("intensities size: %lu",size);
